...
#### 정적 시크릿을 앱에 전달하는 두 축 — "환경변수 주입"과 "API 캐싱"은 서로 다른 문제다

정적 시크릿을 다룰 때 "환경변수로 준다"와 "캐싱해서 부하를 줄인다"는 얼핏 같은 얘기처럼 보이지만 공식문서 기준으로는 **서로 다른 축의 문제**다. 실무에서는 Vault Agent 하나가 이 둘을 동시에 해결해주는 경우가 많아 섞여 보일 뿐이다.

- **축 1 — "앱이 Vault를 알아야 하는가"**: Vault Agent의 **Template** 기능(`template` 또는 `env_template` 블록)은 KV 등 정적 시크릿을 파일로 렌더링하거나 프로세스 환경변수로 만들어 **Process Supervisor 모드(`exec` 블록)**로 실행되는 애플리케이션 프로세스에 주입한다. 공식 튜토리얼("Export secrets as environment variables with Vault Agent")은 전통적 방식(배포/cloud-init 스크립트가 최초 1회 환경변수를 세팅)의 한계를 이렇게 짚는다 — "Traditionally, shell scripts or cloud-init scripts set the environment variables before the application starts. This approach has limitations, such as the need to manually update the environment variables when the secrets change." 그리고 Agent 방식의 결론은 "This application does not need to know about Vault." / "Vault Agent can handle the authentication and secrets retrieval so that your application can remain Vault unaware." — 즉 이 축의 핵심 이점은 **앱 코드에서 Vault SDK/API 호출 자체를 제거**하는 것이지, 부하 절감이 1차 목적이 아니다.
  - `staticSecretRenderInterval` 설정으로 비-lease 시크릿(KV v2 등)을 주기적으로 재렌더링하며, 값이 바뀌면 `restart_on_secret_changes` 등으로 프로세스를 재시작해 새 값을 반영할 수 있다.
- **축 2 — "Vault 서버 호출 빈도를 줄이는가"**: 앞의 "요청 경로에서 매번 Vault를 직접 호출하는 패턴" 절에서 다룬 내용 — 앱이 API 형태로 매 요청마다 Vault를 호출하는 구조라면, Agent/Proxy의 **caching** 기능(토큰·리스는 Agent, 정적 KV는 Enterprise 전용 Proxy)이 이를 흡수한다.
- **두 축이 겹치는 지점**: env_template로 환경변수를 주입하는 방식도 결국 Agent가 시크릿을 로컬에 쥐고 있다가 필요할 때만 다시 조회하는 구조이므로, 결과적으로 "앱의 Vault 비의존화"와 "서버 호출 빈도 감소"를 같은 메커니즘(Agent)으로 함께 달성한다. 다만 이건 이벤트 구독 기반 즉시 무효화가 아니라 **주기적 재조회(polling)**에 가깝다는 점에서, Enterprise Proxy의 static secret caching(이벤트 구독형 실시간 무효화)과는 신선도(freshness) 보장 수준이 다르다.

**질문에 대한 결론**: "정적 시크릿 조회 시 Proxy/로컬 캐싱이 베스트 프랙티스인가"에 대해, 공식문서는 명시적으로 "그렇다"는 취지다 — 앞서 인용한 Vault Agent Caching 튜토리얼이 매 요청마다 직접 호출하는 패턴을 스케일링/레이턴시 문제로 지목하고 캐싱을 대안으로 제시한다. 다만 어떤 캐싱을 쓸지는 상황에 따라 갈린다:

| 상황 | 권장 패턴 |
|---|---|
| 앱을 Vault로부터 완전히 분리하고 싶고, 시크릿 변경이 재배포/재시작 주기로 반영돼도 충분 | **Vault Agent Template** → 환경변수(`env_template`)/파일 렌더링 |
| 앱이 Vault API 형태 호출을 유지하되 다수 인스턴스의 중복 요청을 줄이고 싶고, Enterprise 라이선스가 있음 | **Vault Proxy의 static secret caching** (이벤트 구독 기반 실시간 무효화) |
| OSS이고 앱이 런타임 중 API 호출 형태를 유지해야 함 | 공식 캐싱 기능 없음 — 앱 자체 in-memory 캐시 + TTL/폴링을 직접 구현해야 한다(공식문서가 이 경우의 구체적 레시피를 제공하지는 않아 "확실하지 않음"으로 남긴다) |
...
- https://developer.hashicorp.com/vault/tutorials/vault-agent/agent-env-vars (Vault Agent로 정적 시크릿을 환경변수로 export — 전통적 방식의 한계와 "앱이 Vault를 몰라도 됨"이 목적임을 명시)
- https://developer.hashicorp.com/vault/docs/agent-and-proxy/agent/process-supervisor (`env_template`/`exec` 블록을 이용한 Process Supervisor 모드, `restart_on_secret_changes`)
